...
void updateWorld()
{
	...
	for (i = 0; i < kNumBalls; i++)
        for (j = i+1; j < kNumBalls; j++)
        {
            ...
            {   
                vec3 normalDiff = Normalize(difference);
                vec3 vDelta = VectorSub(ball[i].v, ball[j].v);

                impulse = -(elasticity+1)*DotProduct(normalDiff,vDelta);

                ball[i].P = VectorAdd(ScalarMult(normalDiff,impulse/(1.0/ball[i].mass+1.0)),ball[i].P);
                ball[j].P = VectorAdd(ScalarMult(normalDiff,-impulse/(1.0/ball[j].mass+1.0)),ball[j].P);
            }
        }

	// Control rotation here to reflect
    // friction against floor, simplified as well as more correct
    vec3 up = SetVector(0.0, -kBallSize,0.0);
	for (i = 0; i < kNumBalls; i++)
	{
        // v_rel = ball_v*n
        vec3 relativeVelocity = VectorAdd(ball[i].v, CrossProduct(ball[i].omega,up));  
        vec3 friction = ScalarMult(relativeVelocity,-1);
        ball[i].T = VectorAdd(ball[i].T, CrossProduct(up,friction));
